...
namespace MessagePack.Internal
{
    internal static class FarmHash
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void WeakHashLen32WithSeeds(ulong w, ulong x, ulong y, ulong z, ulong a, ulong b,
            out ulong first, out ulong second)
        {
            unchecked
            {
                a += w;
        ...
        private static unsafe ulong Hash64NA(byte* s, uint len)
        {
            ...
            unchecked
            {
                ...
                WeakHashLen32WithSeeds(s + 32, z + w_second, y + Fetch64(s + 16), out w_first, out w_second);
                swap(ref z, ref x);
                return HashLen16(HashLen16(v_first, w_first, mul) + ShiftMix(y) * k0 + z,
                    HashLen16(v_second, w_second, mul) + x, mul);
            }
        }
        ...
        private static unsafe ulong Hash64UO(byte* s, uint len)
        {
            ...
            unchecked
            {
                ...
                WeakHashLen32WithSeeds(s, v_second * mul, x + w_first, out v_first, out v_second);
                WeakHashLen32WithSeeds(s + 32, z + w_second, y + Fetch64(s + 16), out w_first, out w_second);
                return H(HashLen16(v_first + x, w_first ^ y, mul) + z - u, H(v_second + y, w_second + z, k2, 30) ^ x,
                    k2, 31);
            }
        }
