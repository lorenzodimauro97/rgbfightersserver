        ...
        ref partial struct MessagePackReader
    {
            ...
            switch (code)
            {
                ...
                default:
                    ...
                    if (code >= MessagePackCode.MinFixStr && code <= MessagePackCode.MaxFixStr)
                        return TryGetStringLengthInBytes(out length) && reader.TryAdvance(length);
        ...
        public float ReadSingle()
        {
            ...
            switch (code)
            {
                ...
                default:
                    ...
                    else if (code >= MessagePackCode.MinFixInt && code <= MessagePackCode.MaxFixInt) return code;
        ...
        public DateTime ReadDateTime(ExtensionHeader header)
        {
            if (header.TypeCode != ReservedMessagePackExtensionTypeCode.DateTime)
                throw new MessagePackSerializationException(
                    string.Format("Extension TypeCode is invalid. typeCode: {0}", header.TypeCode));
            ...
            switch (header.Length)
            {
                ...
                case 12:
                    ...
                    ThrowInsufficientBufferUnless(reader.TryReadBigEndian(out longValue));
                    return DateTimeConstants.UnixEpoch.AddSeconds(longValue)
                        .AddTicks(nanoseconds / DateTimeConstants.NanosecondsPerTick);
                default:
                    throw new MessagePackSerializationException(
        ...
        private static Exception ThrowInvalidCode(byte code)
        {
            throw new MessagePackSerializationException(string.Format("Unexpected msgpack code {0} ({1}) encountered.",
                code, MessagePackCode.ToFormatName(code)));
        }
...
#if NETCOREAPP2_1
                initializedChars +=
 decoder.GetChars(this.reader.UnreadSpan.Slice(0, bytesRead), charArray.AsSpan(initializedChars), flush);
#else
                unsafe
                {
