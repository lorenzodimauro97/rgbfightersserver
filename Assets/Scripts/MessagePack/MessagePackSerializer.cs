...
namespace MessagePack
{
    ...
    [SuppressMessage("ApiDesign", "RS0026:Do not add multiple public overloads with optional parameters",
        Justification = "Each overload has sufficiently unique required parameters.")]
    public static partial class MessagePackSerializer
    {
        ...
        /// <param name="options">The options. Use <c>null</c> to use default options.</param>
        /// <exception cref="MessagePackSerializationException">Thrown when any error occurs during serialization.</exception>
        public static void Serialize<T>(ref MessagePackWriter writer, T value,
            MessagePackSerializerOptions options = null)
        {
            options = options ?? DefaultOptions;
            var originalOldSpecValue = writer.OldSpec;
            if (options.OldSpec.HasValue) writer.OldSpec = options.OldSpec.Value;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (array == null) scratchArray = array = new byte[65536];
            ...
            using (var sequenceRental = SequencePool.Shared.Rent())
            {
                ...
                catch (Exception ex)
                {
                    throw new MessagePackSerializationException(
                        "Error occurred while writing the serialized data to the stream.", ex);
                }
            }
        }
        ...
        /// <exception cref="MessagePackSerializationException">Thrown when any error occurs during serialization.</exception>
        public static async Task SerializeAsync<T>(Stream stream, T value, MessagePackSerializerOptions options = null,
            CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            using (var sequenceRental = SequencePool.Shared.Rent())
        ...
        public static T Deserialize<T>(in ReadOnlySequence<byte> byteSequence,
            MessagePackSerializerOptions options = null, CancellationToken cancellationToken = default)
        {
                    ...
                    using (var msgPackUncompressedRental = SequencePool.Shared.Rent())
                    {
                        ...
                        if (TryDecompress(ref reader, msgPackUncompressed))
                        {
                            ...
                            return options.Resolver.GetFormatterWithVerify<T>()
                                .Deserialize(ref uncompressedReader, options);
                        }
        ...
        /// <returns>The deserialized value.</returns>
        /// <exception cref="MessagePackSerializationException">Thrown when any error occurs during deserialization.</exception>
        public static T Deserialize<T>(ReadOnlyMemory<byte> buffer, out int bytesRead,
            CancellationToken cancellationToken = default)
        {
        ...
        public static T Deserialize<T>(ReadOnlyMemory<byte> buffer, MessagePackSerializerOptions options,
            out int bytesRead, CancellationToken cancellationToken = default)
        {
            var reader = new MessagePackReader(buffer)
            {
                CancellationToken = cancellationToken
        ...
        ///     If <see cref="Stream.CanSeek" /> is true on the stream, its position will be set to just after the last
        ///     deserialized byte.
        /// </param>
        /// <param name="options">The options. Use <c>null</c> to use default options.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        ...
        /// <exception cref="MessagePackSerializationException">Thrown when any error occurs during deserialization.</exception>
        /// <remarks>
        ///     If multiple top-level msgpack data structures are expected on the stream, use
        ///     <see cref="MessagePackStreamReader" /> instead.
        /// </remarks>
        public static T Deserialize<T>(Stream stream, MessagePackSerializerOptions options = null,
            CancellationToken cancellationToken = default)
        {
            if (TryDeserializeFromMemoryStream(stream, options, cancellationToken, out T result)) return result;
            ...
            using (var sequenceRental = SequencePool.Shared.Rent())
            {
                ...
                try
                {
                    ...
                    {
                        cancellationToken.ThrowIfCancellationRequested();
                        var span = sequence.GetSpan(stream.CanSeek
                            ? (int) Math.Min(MaxHintSize, stream.Length - stream.Position)
                            : 0);
                ...
                return DeserializeFromSequenceAndRewindStreamIfPossible<T>(stream, options, sequence,
                    cancellationToken);
            }
        }
        ...
        ///     <see cref="MessagePackStreamReader" /> instead.
        /// </remarks>
        public static async ValueTask<T> DeserializeAsync<T>(Stream stream, MessagePackSerializerOptions options = null,
            CancellationToken cancellationToken = default)
        {
            if (TryDeserializeFromMemoryStream(stream, options, cancellationToken, out T result)) return result;
            ...
            using (var sequenceRental = SequencePool.Shared.Rent())
            {
                ...
                try
                {
                    ...
                    do
                    {
                        var memory = sequence.GetMemory(stream.CanSeek
                            ? (int) Math.Min(MaxHintSize, stream.Length - stream.Position)
                            : 0);
        ...
        private static bool TryDeserializeFromMemoryStream<T>(Stream stream, MessagePackSerializerOptions options,
            CancellationToken cancellationToken, out T result)
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (stream is MemoryStream ms && ms.TryGetBuffer(out var streamBuffer))
            {
                result = Deserialize<T>(streamBuffer.AsMemory(checked((int) ms.Position)), options, out var bytesRead,
                    cancellationToken);
        ...
        private static T DeserializeFromSequenceAndRewindStreamIfPossible<T>(Stream streamToRewind,
            MessagePackSerializerOptions options, ReadOnlySequence<byte> sequence, CancellationToken cancellationToken)
        {
            if (streamToRewind is null) throw new ArgumentNullException(nameof(streamToRewind));
        ...
        private static int LZ4Operation(in ReadOnlySequence<byte> input, Span<byte> output, LZ4Transform lz4Operation)
        {
            ...
            {
                if (rentedInputArray != null) ArrayPool<byte>.Shared.Return(rentedInputArray);
            }
        }
            ...
            if (!reader.End)
            {
                ...
                if (reader.NextMessagePackType == MessagePackType.Extension)
                {
                    ...
                    if (header.TypeCode == ThisLibraryExtensionTypeCodes.Lz4Block)
                    {
                        ...
                        var uncompressedLength = extReader.ReadInt32();
                        ...
                        var actualUncompressedLength = LZ4Operation(compressedData, uncompressedSpan, LZ4CodecDecode);
                        Debug.Assert(actualUncompressedLength == uncompressedLength,
                            "Unexpected length of uncompressed data.");
                ...
                if (reader.NextMessagePackType == MessagePackType.Array)
                {
                    ...
                    if (arrayLength != 0 && peekReader.NextMessagePackType == MessagePackType.Extension)
                    {
                        ...
                        if (header.TypeCode == ThisLibraryExtensionTypeCodes.Lz4BlockArray)
                        {
                            ...
                            try
                            {
                                for (var i = 0; i < sequenceCount; i++) uncompressedLengths[i] = reader.ReadInt32();

                                for (var i = 0; i < sequenceCount; i++)
                                {
                                    var uncompressedLength = uncompressedLengths[i];
                                    var lz4Block = reader.ReadBytes();
                                    var uncompressedSpan =
                                        writer.GetSpan(uncompressedLength).Slice(0, uncompressedLength);
                                    var actualUncompressedLength =
                                        LZ4Operation(lz4Block.Value, uncompressedSpan, LZ4CodecDecode);
                                    Debug.Assert(actualUncompressedLength == uncompressedLength,
                                        "Unexpected length of uncompressed data.");
                                    writer.Advance(actualUncompressedLength);
                                }
        ...
        private static void ToLZ4BinaryCore(in ReadOnlySequence<byte> msgpackUncompressedData,
            ref MessagePackWriter writer, MessagePackCompression compression)
        {
            if (compression == MessagePackCompression.Lz4Block)
            {
                ...
                try
                {
                    var lz4Length = LZ4Operation(msgpackUncompressedData, lz4Span, LZ4CodecEncode);

                    const int LengthOfUncompressedDataSizeHeader = 5;
                    writer.WriteExtensionFormatHeader(new ExtensionHeader(ThisLibraryExtensionTypeCodes.Lz4Block,
                        LengthOfUncompressedDataSizeHeader + (uint) lz4Length));
            ...
            else if (compression == MessagePackCompression.Lz4BlockArray)
            {
                ...
                writer.WriteExtensionFormatHeader(new ExtensionHeader(ThisLibraryExtensionTypeCodes.Lz4BlockArray,
                    extHeaderSize));
                {
                    foreach (var item in msgpackUncompressedData) writer.Write(item.Length);
                }
                ...
                foreach (var item in msgpackUncompressedData)
                {
                    ...
                    var lz4Length = LZ4Codec.Encode(item.Span, lz4Span.Slice(5, lz4Span.Length - 5));
