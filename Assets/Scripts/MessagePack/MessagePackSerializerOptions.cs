...
namespace MessagePack
{
    ...
    public class MessagePackSerializerOptions
    {
        ...
        internal static readonly Regex AssemblyNameVersionSelectorRegex = new Regex(
            @", Version=\d+.\d+.\d+.\d+, Culture=[\w-]+, PublicKeyToken=(?:null|[a-f0-9]{16})$", RegexOptions.Compiled);
        ...
        public static MessagePackSerializerOptions Standard =>
            MessagePackSerializerOptionsDefaultSettingsLazyInitializationHelper.Standard;
#endif
        ...
        ///     and serialization may not compress if msgpack sequences are short enough that compression would not likely be
        ///     advantageous.
        /// </remarks>
        ...
        /// <summary>
        ///     Gets a value indicating whether serialization should omit assembly version, culture and public key token metadata
        ///     when using the typeless formatter.
        /// </summary>
        /// <value>The default value is <c>false</c>.</value>
        public bool OmitAssemblyVersion { get; private set; }

        /// <summary>
        ///     Gets a value indicating whether deserialization may instantiate types from an assembly with a different version if
        ///     a matching version cannot be found.
        /// </summary>
        /// <value>The default value is <c>false</c>.</value>
        ...
        /// </summary>
        /// <param name="typeName">
        ///     The name of the type to load. This is typically the <see cref="Type.AssemblyQualifiedName" />
        ///     but may use the assembly's simple name.
        /// </param>
        ...
        public virtual Type LoadType(string typeName)
        {
            ...
            if (result == null && AllowAssemblyVersionMismatch)
            {
                var shortenedName = AssemblyNameVersionSelectorRegex.Replace(typeName, string.Empty);
                if (shortenedName != typeName) result = Type.GetType(shortenedName, false);
            }
        ...
        /// <remarks>
        ///     This method provides a means for an important security mitigation when using the Typeless formatter to prevent
        ///     untrusted messagepack from
        ...
        ///     Applications that deserialize from untrusted sources should override this method and throw if the type is not among
        ///     the expected set.
        /// </remarks>
        public virtual void ThrowIfDeserializingTypeIsDisallowed(Type type)
        {
            if (BlacklistCheck.Contains(type.FullName))
                throw new MessagePackSerializationException("Deserialization attempted to create the type " +
                                                            type.FullName + " which is not allowed.");
        }
        ...
        {
            if (Resolver == resolver) return this;
        ...
        {
            if (Compression == compression) return this;
        ...
        {
            if (OldSpec == oldSpec) return this;
        ...
        {
            if (OmitAssemblyVersion == omitAssemblyVersion) return this;
        ...
        {
            if (AllowAssemblyVersionMismatch == allowAssemblyVersionMismatch) return this;
        ...
        public MessagePackSerializerOptions WithSecurity(MessagePackSecurity security)
        {
            ...
            if (Security == security) return this;
        ...
        protected virtual MessagePackSerializerOptions Clone()
        {
            if (GetType() != typeof(MessagePackSerializerOptions))
                throw new NotSupportedException(
                    $"The derived type {GetType().FullName} did not override the {nameof(Clone)} method as required.");
