...
namespace MessagePack
{
    ...
    /// </summary>
    /// <remarks>
    ///     This class is *not* thread-safe. Do not call more than one member at once and be sure any call completes (including
    ///     asynchronous tasks)
    ...
    public class MessagePackStreamReader : IDisposable
    {
        ...
        /// </summary>
        /// <param name="stream">
        ///     The stream to read from. This stream will be disposed of when this
        ///     <see cref="MessagePackStreamReader" /> is disposed.
        /// </param>
        ...
        ///     If true, leaves the stream open after this <see cref="MessagePackStreamReader" /> is disposed;
        ///     otherwise, false.
        /// </param>
        ...
        ///     <see cref="ReadAsync(CancellationToken)" />.
        /// </summary>
        public ReadOnlySequence<byte> RemainingBytes => endOfLastMessage.HasValue
            ? ReadData.AsReadOnlySequence.Slice(endOfLastMessage.Value)
            : ReadData.AsReadOnlySequence;
        ...
        ///     any extra bytes read (between the last complete message and the end of the stream) will be available via the
        ///     <see cref="RemainingBytes" /> property.
        /// </remarks>
        public async ValueTask<ReadOnlySequence<byte>?> ReadAsync(CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                if (!await TryReadMoreDataAsync(cancellationToken).ConfigureAwait(false))
                {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            var buffer = ReadData.GetMemory(0);
            var bytesRead = 0;
            try
            {
        ...
        private bool TryReadNextMessage(out ReadOnlySequence<byte> completeMessage)
        {
            ...
            {
                var reader = new MessagePackReader(ReadData);
