...
namespace MessagePack
{
        ...
        ref struct MessagePackWriter
    {
        ...
        public void Write(DateTime dateTime)
        {
            if (OldSpec)
                throw new NotSupportedException(
                    $"The MsgPack spec does not define a format for {nameof(DateTime)} in {nameof(OldSpec)} mode. Turn off {nameof(OldSpec)} mode or use NativeDateTimeFormatter.");
            // Timestamp spec
        ...
        {
            var bufferLength = estimatedOffset + byteCount;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void MemoryCopy(void* source, void* destination, long destinationSizeInBytes,
