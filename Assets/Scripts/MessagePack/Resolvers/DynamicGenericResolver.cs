        ...
        internal static object GetFormatter(Type t)
        {
            ...
            if (t.IsArray)
            {
                ...
                if (rank == 2)
                    return Activator.CreateInstance(
                        typeof(TwoDimensionalArrayFormatter<>).MakeGenericType(t.GetElementType()));
                if (rank == 3)
                    return Activator.CreateInstance(
                        typeof(ThreeDimensionalArrayFormatter<>).MakeGenericType(t.GetElementType()));
                if (rank == 4)
                    return Activator.CreateInstance(
                        typeof(FourDimensionalArrayFormatter<>).MakeGenericType(t.GetElementType()));
            ...
            if (ti.IsGenericType)
            {
                ...
                if (ti.GenericTypeArguments.Length == 1
                    && ti.ImplementedInterfaces.Any(x =>
                        x.GetTypeInfo().IsConstructedGenericType() &&
            ...
            else
            {
                ...
                if (t == typeof(IDictionary)) return NonGenericInterfaceDictionaryFormatter.Instance;

                if (typeof(IList).GetTypeInfo().IsAssignableFrom(ti) &&
                    ti.DeclaredConstructors.Any(x => x.GetParameters().Length == 0))
