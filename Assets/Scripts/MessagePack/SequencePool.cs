...
namespace MessagePack
{
    ...
    internal class SequencePool
    {
        ...
        /// </summary>
        /// <remarks>
        ///     We use a <see cref="maxSize" /> that allows every processor to be involved in messagepack serialization
        ///     concurrently,
        ...
        /// <summary>
        ///     The array pool which we share with all <see cref="Sequence{T}" /> objects created by this
        ///     <see cref="SequencePool" /> instance.
        /// </summary>
        /// <devremarks>
        ///     We allow 100 arrays to be shared (instead of the default 50) and reduce the max array length from the default 1MB
        ///     to something more reasonable for our expected use.
        /// </devremarks>
        ...
        internal Rental Rent()
        {
            lock (pool)
            {
                if (pool.Count > 0) return new Rental(this, pool.Pop());
            }
            ...
            return new Rental(this, new Sequence<byte>(arrayPool) {MinimumSpanLength = MinimumSpanLength});
        }
